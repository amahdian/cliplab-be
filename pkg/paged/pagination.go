...
func NewPaginatedResponse(data interface{}, dataCount, totalCount, page, limit int, sort string) *PaginatedResponse {
	pr := PaginatedResponse{
		...
		First:            page == 0,
		Last:             page >= int(math.Ceil(float64(totalCount)/float64(limit)))-1,
	}
	//Pagination detail
	pr.Pageable.PageNumber = page
	pr.Pageable.PageSize = limit
	pr.Pageable.Offset = page * limit
